...
begin
  ...
  Pip.V1old:=Pip.V1;
  { Current-Clamp: }
  if (ANrn_.NP.If_I_V_or_g in [2,6,7]) then begin
      if          ANrn_.NP.HH_order='1-point'  then begin
         Pip.i2:=-(ssE+ssI)*(Pip.V1-Vus) + uuE+uuI + Iind;
         uuE:=0; ssE:=0; uuI:=0; ssI:=0;
      end else if (ANrn_.NP.HH_order='2-points') or
                  (ANrn_.NP.HH_order='2-p-MainenSejn') or
                  (ANrn_.NP.HH_order='2-p-ActiveDend') then begin
