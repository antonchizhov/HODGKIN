...
procedure SynCurrentOnNewStep(Ampl,mean :double; var Curr :double);
...
var x :double;
begin
  x:=Ampl*NoiseToSignal*randG(0,1);
  if tau_Noise=0 then begin
     Curr:= mean+max(-10*Ampl*NoiseToSignal,min(x,10*Ampl*NoiseToSignal));
  end else begin
     Curr:= Curr + (mean-Curr)/tau_Noise*dt + x*sqrt(2*dt/tau_Noise);
  end;
end;
