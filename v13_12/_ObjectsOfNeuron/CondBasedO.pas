...
uses
  ...
  Calmar_NaO,Migli_NaO,Shu_NaO,Mainen_NaO,Mainen_NaRO,White_NaO,White2_NaO,Naundorf_NaO,
  Chow_NaO,Cum_NaO,
...
{==================================================================}
  ...
  public
    ...
    procedure Calc_RHP_and_g(var ActiveRHP,g :double); override;
...
begin
  ...
  END ELSE BEGIN
     ...
     else if NP.Na_type='Mainen'   then  chNa:=TMainenNa.Create(self)
     ...
     else if NP.NaR_type='Mainen'   then  chNaR:=TMainenNaR.Create(self)
...
begin
  ...
  chNaP:=TKononNaP.Create(self);      {Turned on 10.07.2012}
...
function TCondBasedNrn.fNMDA(V,Mg :double) :double;
begin
...
procedure TCondBasedNrn.Calc_RHP_and_g(var ActiveRHP,g :double);
...
begin
      ...
      {***}
      NV.IK_tot:=NV.IK+NV.IKA+NV.IKM+NV.IKD+NV.IKCa+NV.IAHP + NV.I_leak;
      ActiveRHP:= -NV.INaR -NV.INa -NV.IK -NV.IKM -NV.IKA -NV.IKD
...
begin
  NV.V:=NP.Vrest;  NV.VatE:=NP.Vrest; NV.DVDt:=0;
...
var
     indic_                       :integer;
     DVDt_old,Th_inf,
     DIsynE,V_E,V_0,gs,gdE,
     ActiveRHP_,g_,
     dVE,dV0,I_dend_to_soma       :double;
     Status                       :string;
BEGIN
  NV.Vold:=NV.V;
  DVDt_old:=NV.DVDt;
  { ---------------- Currents ----------------------------------- }
      Calc_RHP_and_g(ActiveRHP_,g_);
      NV.Im:= ActiveRHP_ - NV.I_leak + Iind;
  { ---------------- Ohm's law -------------------------------------- }
  if          NP.HH_order='1-point'  then begin
      ...
      repeat
        NV.gNMDA:= tt*fNMDA(NV.V{atE},NP.Mg);
//22.03.2013        NV.Isyn:= -(ssE+ssI)*(NV.V-Vus) + uuE+uuI - NV.gNMDA*(NV.V-NP.VNMDA);
        ...
        I_dend_to_soma:=NV.IsynE;//gdE*(NV.VatE -NV.V); {moved and changed on 22.03.2013}
        {----------}
        dVE := dt/NP.tau_m0*(-(NV.VatE -NP.Vrest)
                 - (2+gdE/gs)/NP.l*(NV.VatE -NV.V)            + 2*NV.IatE/gdE
                             -(NV.V    -NP.Vrest)  - NV.Im/gs -   NV.IsynI/gs); {introduced on 7.10.2013}
        {----------}
        dV0 := dt/NP.tau_m0*(
                      gdE/gs/NP.l *(NV.VatE -NV.V) + NV.Im/gs +   NV.IsynI/gs);
        {----------}
        NV.VatE:=V_E + dVE;
        NV.V   :=V_0 + dV0;
        if Status='Predictor' then begin
           NV.VatE:=(NV.VatE+V_E)/2;
           NV.V   :=(NV.V   +V_0)/2;
  ...
  NV.PSC:=-(NV.Isyn + NV.Im + I_dend_to_soma - NP.C_membr*NV.DVDt)*NP.Square*1e9; {pA}
  //CalculateActiveConductance;
