...
begin
 ...
 with NP do BEGIN
  ...
  VKDr:=0;  VKM:=0;  VKD:=0;    VADP:=0.120{V};  VK:=-0.070 {V};
...
//  If_I_V_or_g:=2;    {26.09.2011}
  ...
  if          HH_type='Calmar' then begin
     ...
     if HH_order='2-points' then Square:=Square*(3+ro)/(3+2*ro); // [p.19]
     gNa :=    30 {mS/cm^2};
     gK  :=    40 {mS/cm^2};
     gL  :=  0.1 {mS/cm^2};
     Vrest:=-0.060 {V};
     ...
     if HH_order='2-points' then n_AP:=0.66;
     dT_AP:=0.0014{s};
     IfSet_gL_or_tau:=1;
     IfSet_VL_or_Vrest:=2;
     ...
     gNaR:=    20 {mS/cm^2};
     gNa :=    20 {mS/cm^2};
     gK  :=     2 {mS/cm^2};  //It was 0.15 before Feb2013
     gL  :=  0.05 {mS/cm^2};
     Vrest:=-0.065 {V};
     Tr :=0 {mV !};
     VNa:= 0.060 {V};
     ...
     if HH_order='2-points' then n_AP:=1;
     dT_AP:=0.0014{s};
     IfSet_gL_or_tau:=1;
     IfSet_VL_or_Vrest:=2;
     IfBlockNaR:=0;
     ...
     if HH_order='2-points' then Square:=tau_m0/C_membr/39000{KOhm}*(3+ro)/(3+2*ro); // [p.19]
     gNaR  :=  0 {mS/cm^2};
     gNa :=    2.28 {mS/cm^2};
     gK  :=    0.76 {mS/cm^2};
     gKA :=    4.36 {mS/cm^2};
     ...
     VKD :=-0.095 {V};
     V12H:=-0.017 {V};
     Temperature:=309;//297 {K};       {[p.104 Lyle]}
