...
interface
uses Forms,Math,
     MyTypes,Init,t_dtO,MathMyO,Graph,Other,FiringClamp,Hodgkin,
...
var ni,nIind{,indicat }                         :integer;
    hhh                                         :text;
    Iind_mem,u_mem,s_mem,sgm_V,gL0,Noise_       :double;
    ANrnF,ANrnT                                 :TNeuron;
BEGIN
...
var ns,nu,nt_end_rem,imem                         :integer;
    ...
    t_end_rem,Iind_mem,u_mem,s_mem,uu,ss,x,y,fmem :double;
    IfNoise                                       :boolean;
    ANrnF,ANrnT                                   :TNeuron;
BEGIN
...
//      MembranePotential;
//      NoSpikePotential(t-NV.tAP,ThrNeuron,NV.DVDt,NV.ddV);
      ANrnF.MembranePotential(uu,ss,0,s_soma,tt,0,0);
      ANrnT.MembranePotential(uu,ss,0,s_soma,tt,0,0);
    ...
    REPEAT  nt:=nt+1;  t:=nt*dt; { time step }
      ...
      if IfNoise then begin
        if tau_Noise=0 then begin
           ...
           Ampl_:=sgm_V*gL0*sqrt(1+NP0.tau_m0/tau_Noise);
